        ...
        if activation_bouton_verif_simul == 0 :
            ...
            # Échec de l'utilisateur
                ...
                for num_espece, population in enumerate(populations_finales):
                    if population < 10:
                        st.markdown(f"<span style='font-size: 16px;'>❌ L'espèce **{noms[num_espece]}** s'est éteinte.</span>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<span style='font-size: 15px;'>✅ L'espèce **{noms[num_espece]}** a survécu.</span>", unsafe_allow_html=True)
